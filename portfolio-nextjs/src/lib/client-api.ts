...
import { ApiResponse, GuestbookEntry, UmamiStats } from "../components";
...
export const guestbookApi = {
  ...
  createEntry: async (content: string) => {
    const res = await api.post<ApiResponse<RawGuestbookEntry>>("/guestbook", {
      content,
    });
    return res.data;
  },
...
export const todoApi = {
  ...
  update: async (id: string, updates: Partial<Pick<Todo, "text" | "done">>) => {

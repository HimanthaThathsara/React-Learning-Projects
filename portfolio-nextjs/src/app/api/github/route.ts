...
class GitHubStatsCalculator {
  ...
  //  Top Languages
  static calculateTopLanguages(repositories: Array<{isFork: boolean; primaryLanguage?: {name: string; color: string}}>): Language[] {
  ...
  static countRepositoryTypes(repositories: Array<{isFork: boolean}>): {original: number; forked: number} {
  ...
  static countPullRequestStates(pullRequests: Array<{state: string}>): {open: number; closed: number; merged: number} {
  ...
  static calculateWeeklyContributions(weeks: Week[]): number {
    const allDays = weeks.flatMap((w) => w.contributionDays);
    return allDays.filter((d) => dayjs(d.date).isAfter(dayjs().subtract(7, "day")) && dayjs(d.date).isBefore(dayjs())).reduce((sum, d) => sum + d.contributionCount, 0);
  }
  ...
  static estimateWeeklyStars(repositories: Array<{createdAt: string; stargazerCount: number}>): number {
    return repositories.filter((r) => this.isWithinDays(r.createdAt, 7)).reduce((sum, r) => sum + r.stargazerCount, 0);
  }
...
export async function GET() {
  try {
    ...
    // Weekly trends
    const weeklyTrends = GitHubStatsCalculator.calculateWeeklyTrends(repositories, pullRequests, weeks, totalRepos, totalStars, totalContrib, totalPRs);

...
import {BookOpenText, X, Send, Github, Trash2, AlertCircle, Crown} from "lucide-react";
import {useState} from "react";
import {cn, parseMessageWithMentions} from "../../lib/utils";
import {useGuestbookEntries, useCreateGuestbookEntry, useDeleteGuestbookEntry} from "../../hooks/use-guestbook";
    ...
    <p className="text-foreground/80 mb-2 text-sm leading-relaxed">
      {parts.map((part) => {
        if (part.type === "mention") {
          return (
            <span key={part.key} className="bg-primary/10 text-primary border-primary/20 hover:bg-primary/20 rounded border px-1 font-semibold transition-colors">
              {part.content}
            </span>
          );
        }
            ...
            <motion.div
              ...
              <div className="relative shrink-0 border-b-2 p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="font-incognito text-2xl font-bold">Guestbook</h2>
                    <p className="text-muted-foreground mt-1 font-mono text-xs">
                      {"<"} Leave a message {"/>"}
                    </p>
                  </div>
                  <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="border-2">
                    <X className="h-5 w-5" />
                  </Button>
                </div>
                ...
                {error && (
                  <motion.div initial={{opacity: 0, y: -10}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -10}} className="mx-4 mt-4">
                    <div className="flex items-center gap-2 rounded-lg border-2 border-red-500/50 bg-red-500/10 p-3 text-sm text-red-500">
                      ...
                      <button onClick={() => setError(null)} className="rounded p-1 hover:bg-red-500/20">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  </motion.div>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {isLoading ? (
                  <div className="flex h-full flex-col items-center justify-center">
                    ...
                    <p className="text-muted-foreground mt-4 font-mono text-sm">Loading messages...</p>
                  </div>
                ) : (entries?.length ?? 0) === 0 ? (
                          ...
                          <div className="bg-muted/20 hover:bg-muted/40 rounded-lg border-2 border-dashed p-4 transition-colors">
                            <div className="flex gap-3">
                              ...
                              <div className="relative size-10 flex-shrink-0">
                                <img
                                  src={entry.user.image || `https://ui-avatars.com/api/?name=${entry.user.name || "User"}`}
                              ...
                              <div className="flex-1">
                                <div className="mb-1 flex flex-wrap items-center gap-2">
                                  <span className="font-incognito font-semibold">{entry.user.name || "Anonymous"}</span>
                                  {isEntryAuthor && (
                                    <Badge variant="default" className="gap-1 border-green-500/30 bg-green-500/10 font-mono text-[10px] text-green-600">
                                ...
                                <span className="text-muted-foreground font-mono text-xs">{formatTimestamp(entry.createdAt)}</span>
                              </div>

                              {/* TODO Delete button  */}
                              {isAuthor && (
                                <Button variant="ghost" size="icon" className="h-8 w-8 opacity-0 transition-opacity group-hover:opacity-100" onClick={() => handleDelete(entry.id)}>
              ...
              <div className="bg-background shrink-0 border-t-2 p-4">
                {!isLoggedIn ? (
                  ...
                  <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="space-y-3">
                    <p className="text-muted-foreground mb-3 text-center font-mono text-xs">Sign in to leave a message</p>

                    <div className="flex-center gap-2 max-sm:flex-col">
                      <Button onClick={() => handleSignIn("github")} variant="default" className="group border-2 font-medium" size="lg">
                        <Github className="mr-2 h-4 w-4" />
                        Sign in with GitHub
                      </Button>
                      <Button onClick={() => handleSignIn("google")} variant="outline" className="group border-2 font-medium" size="lg">
                  ...
                  <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="space-y-3">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <span className={cn("text-muted-foreground font-mono text-xs", message.length > 450 && "text-orange-500", message.length >= 500 && "text-red-500")}>{message.length}/500</span>
                      <Button onClick={handleSendMessage} disabled={!message.trim() || isPending} className="group border-2 font-medium" size="lg">

...
import GooeySvgFilter from "../../ui/geogy-svg-filter";
...
import {BackgroundGridAnimated} from "../../shared/backgrounds";
...
const TechStackCard = () => {
  return (
    <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.5, duration: 0.6, ease: "easeOut"}} className="bg-muted/30 relative rounded-xl border-2 p-6">
      {/* Header */}
        ...
        {Object.entries(TECH_STACK).map(([category, techs]) => (
          <div key={category} className="space-y-3">
            ...
            <div className="relative flex items-center justify-center">
              ...
              <span className="text-muted-foreground px-2 text-xs font-medium tracking-widest uppercase">{category}</span>
              <div className="via-primary/40 h-px flex-1 bg-gradient-to-r from-transparent to-transparent" />
            </div>
            ...
            <div className="relative overflow-hidden rounded-xl border border-dashed">
              ...
              <Marquee pauseOnHover>
                {techs.map((tech) => (
                  <div key={tech.name} className="group relative flex h-14 w-14 items-center justify-center rounded-md transition-transform duration-300 hover:scale-110" aria-label={tech.name}>
                    <img
                      src={tech.icon}
                      alt={tech.name}
...
const RobotWithEyes = ({className}: {className?: string}) => {
  ...
  return (
    <div className={cn("relative", className)}>
      <Robot>
        <div className="flex h-full w-full items-center justify-center gap-8 bg-neutral-950" ref={containerRef}>
          <Eyes
            mouseFollow={{
              enabled: true,
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <motion.div
              ...
              <div className="relative z-10">
                <div className="mb-3 flex items-start justify-between">
                  <p className="text-muted-foreground font-mono text-xs tracking-wider uppercase">{stat.label}</p>
        ...
        {/*  Robot */}
        <motion.div initial={{opacity: 0, scale: 0.8}} animate={{opacity: 1, scale: 1}} transition={{delay: 0.3}} className="bg-muted/30 relative overflow-hidden rounded-xl border p-4 max-md:hidden">
          ...
          {overviewStats.slice(2, 4).map((stat, index) => (
            <motion.div
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-2">
                  <p className="text-2xl font-bold">
                    <NumberTicker value={stat.value} delay={(index + 2) * 0.1} />
                  </p>
                  <span className="flex items-center text-xs font-medium text-green-400">
                    <ArrowUpRight className="h-3 w-3" />
                    {stat.trend}
        ...
        <motion.div
          ...
          <div className="relative z-10">
            ...
            <div className="text-3xl font-bold">
              <NumberTicker value={data?.followers?.totalCount ?? 0} delay={0.5} />
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              {(data?.followers?.nodes?.length ?? 0) > 5 && <span className="text-muted-foreground text-xs">+{(data?.followers?.nodes?.length ?? 0) - 5} more</span>}
            </div>
          </div>
        </motion.div>
...
const ActivityContent = ({data}: {data?: GitHubStatsResponse}) => {
  ...
  const years = Array.from({length: currentYear - GITHUB_JOIN_YEAR + 1}, (_, i) => GITHUB_JOIN_YEAR + i).reverse();

  const contributionData = data?.contributionsCollection.contributionCalendar.weeks
  ...
  return (
    <div className="space-y-6">
      {/* GitHub Calendar */}
      <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="bg-muted/30 rounded-xl border-2 p-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground text-xs">
                <NumberTicker value={data?.contributionsCollection?.contributionCalendar?.totalContributions ?? 0} delay={0.1} /> contributions
              </span>
            </div>
          </div>
            ...
            {years.map((year) => (
              <motion.button
                ...
                className={cn(
                  ...
                  selectedYear === year ? "border-green-400 bg-green-400/10 text-green-400" : "bg-muted text-muted-foreground hover:border-neutral-600"
                )}>
                {year}
              </motion.button>
            ))}
        ...
        <div className="overflow-x-auto">
          <div className="max-h-fit max-w-fit">
            <GitHubCalendar username={env.NEXT_PUBLIC_GITHUB_USERNAME} colorScheme={(resolvedTheme as "light" | "dark") ?? "dark"} blockSize={13} year={selectedYear} />
          </div>
        </div>
      </motion.div>

      {/* Activity Chart */}
      <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.1}} className="bg-muted/30 overflow-x-auto rounded-xl border-2 p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 rounded-full bg-green-400/10 px-3 py-1">
            ...
            <span className="text-xs font-medium text-green-400">{(data?.currentStreak ?? 0) > 0 ? "Active" : "Inactive"}</span>
          </div>
        </div>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={contributionData}>
            ...
            />
            <Area type="monotone" dataKey="commits" stroke="#4ade80" strokeWidth={2} fillOpacity={1} fill="url(#colorGradient)" />
          </AreaChart>
        </ResponsiveContainer>
      </motion.div>
    </div>
...
const InsightsContent = ({data}: {data?: GitHubStatsResponse}) => {
  ...
  const originalPercentage = Math.round(((data?.repositories.original ?? 0) / totalRepos) * 100);
  const forkedPercentage = Math.round(((data?.repositories.forked ?? 0) / totalRepos) * 100);
  ...
  const prClosedPercentage = data?.pullRequests.total ? Math.round(((data.pullRequests.closed + data.pullRequests.merged) / data.pullRequests.total) * 100) : 0;

  const issuesClosedPercentage = data?.issues.total ? Math.round((data.issues.closed / data.issues.total) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Metrics Grid */}
      <div className="grid gap-4 md:grid-cols-2">
        <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.2}} className="bg-muted/30 rounded-xl border-2 p-5">
          <h3 className="mb-4 font-semibold">Repository Breakdown</h3>
          <div className="text-muted-foreground mb-4 text-xs">
            Total: <NumberTicker value={data?.repositories.total ?? 0} delay={0.2} /> repositories
          </div>
          <div className="space-y-3">
              ...
              <div key={item.label}>
                <div className="mb-1 flex items-center justify-between">
                  ...
                  <span className="text-muted-foreground text-xs font-medium">
                    ...
                    <NumberTicker value={item.label === "Original" ? (data?.repositories.original ?? 0) : (data?.repositories.forked ?? 0)} delay={0.3 + i * 0.1} />)
                  </span>
                </div>
        ...
        {/* Activity Metrics */}
        <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.3}} className="bg-muted/30 rounded-xl border-2 p-5">
            ...
            {activityMetrics.map((item, i) => (
              <div key={item.label}>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span className="text-muted-foreground text-xs">
                    Total: <NumberTicker value={item.total} delay={0.4 + i * 0.1} />
                  </span>
                </div>
                <div className="bg-muted h-2 rounded-full">
                  <motion.div
                ...
                <div className="text-muted-foreground mt-1 text-xs">{item.breakdown}</div>
              </div>
            ))}
          </div>
        </motion.div>
...
const LoadingState = () => (
  <div className="flex min-h-[400px] items-center justify-center">
    <div className="text-center">
      ...
      <p className="text-muted-foreground mt-4 text-sm">Loading GitHub stats...</p>
    </div>
  </div>
);
    ...
    <SectionHeading className="px-4 py-12 md:px-8" id="stats" text="Stats">
      ...
      <div className="w-full">
        <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="mb-8 text-center">
          <h2 className="font-incognito text-4xl font-semibold">GitHub Statistics</h2>
          <p className="text-muted-foreground mt-2">Tracking my open source journey</p>
        </motion.div>
          ...
          <div className="no-scrollbar relative h-screen w-full overflow-y-auto">
            ...
            <div className="relative flex w-full">
              {TAB_CONTENT.map((tab, index) => (
                <button key={index} onClick={() => setActiveTab(index)} className="group h-10 flex-1 transition-all md:h-12">
                  <span
                    className={cn("flex h-full w-full items-center justify-center gap-2 font-medium transition-all", {
                      "text-foreground": activeTab === index,
                      "text-muted-foreground": activeTab !== index,
                    })}>
                    <tab.icon className={cn("h-4 w-4 transition-transform", activeTab === index && "scale-110")} />
                    {tab.title}
                  </span>
                </button>
              ))}

...
const Navbar = () => {
  ...
  const { resolvedTheme: initialTheme, setTheme } = useTheme();
  const [resolvedTheme, setResolvedTheme] = useState<string | undefined>(
    "dark",
  );

  useEffect(() => {
    setResolvedTheme(initialTheme || "dark");
  }, [initialTheme]);
  ...
  return (
    <nav
      className={`sticky top-0 z-50 w-full border-b px-4 py-2.5 md:px-8 ${resolvedTheme === "dark" ? "bg-black text-white" : "bg-white text-black"}`}
      id="home"
    >

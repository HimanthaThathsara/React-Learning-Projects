...
//  PRESETS
const SIZE_PRESETS: Record<EyeSize, {width: string; height: string; gap: string}> = {
...
export const Eyes = ({className, mouseFollow, classes = {}, size = "md", eyeColor = "#F6EAC5", glow, shape = "normal", lookAround, pause = false}: EyesProps) => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (containerRef.current) {
        ...
        if (mouseFollow.containerRef?.current) {
          const boundRect = mouseFollow.containerRef.current.getBoundingClientRect();
          targetX = Math.max(boundRect.left, Math.min(boundRect.right, targetX));
          targetY = Math.max(boundRect.top, Math.min(boundRect.bottom, targetY));
        }

        const maxX = mouseFollow.maxX ?? 15;
        const maxY = mouseFollow.maxY ?? 10;

        const angleX = Math.max(-maxX, Math.min(maxX, (targetX - centerX) / 15));
        const angleY = Math.max(-maxY, Math.min(maxY, (targetY - centerY) / 15));
  ...
  }, [mouseFollow?.enabled, mouseFollow?.containerRef, mouseFollow?.maxX, mouseFollow?.maxY, pause]);
  ...
  return (
    <div ref={containerRef} className={cn("inline-flex items-center justify-center", sizeConfig.gap, classes.container, className)}>
      {[0, 1].map((index) => (
        <Eye
          key={index}
          index={index}
...
const Eye = React.memo(({index, mousePos, isBlinking, mouseFollow, sizeConfig, eyeColor, glowStyle, glow, shape, classes, lookAround, pause}: EyeProps) => {
  ...
  return (
    <motion.div
      className={cn("relative origin-center ring-2 ring-black", sizeConfig.width, sizeConfig.height, shapeClass, classes.eye)}
      style={{

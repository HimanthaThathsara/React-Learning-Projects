...
const DPad = ({className, onClick}: {className?: string; onClick?: (type: DpadButtonLabel) => void}) => {
  return (
    <div className={cn("absolute z-50 h-[130px] w-[38px] overflow-visible", className)}>
      {/* Left */}
      <DirBtn className="top-[44px] left-[-47px] h-[42px] w-[67px] rounded-tl-[36px] rounded-bl-[36px]" onClick={() => onClick?.("left")} />
      {/* Right */}
      <DirBtn className="top-[44px] left-[18px] h-[42px] w-[67px] rounded-tr-[36px] rounded-br-[36px]" onClick={() => onClick?.("right")} />
      {/* Up */}
      <DirBtn className="top-0 left-0 h-[48px] w-[38px] rounded-tl-[36px] rounded-tr-[36px]" onClick={() => onClick?.("up")} />
      {/* Down */}
      <DirBtn className="bottom-0 left-0 h-[48px] w-[38px] rounded-br-[36px] rounded-bl-[36px]" onClick={() => onClick?.("down")} />
...
const PlaydateConsole = ({children, onActionButtionClick, onDpadButtonClick, isPlaying = false}: PlaydateConsoleProps) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      const direction = (Object.keys(mapping) as DpadButtonLabel[]).find((key) => mapping[key] === e.key);
  ...
  const Crank = (
    <div>
      <div
        ...
        />
        <div className="absolute right-[-22px] bottom-[14px] h-[12px] w-[12px] rounded-full border bg-[var(--gb-ui-mid)] shadow-[0_-1px_1px_0_#fff]" style={{borderColor: "var(--gb-crank-border)"}} />
      </div>
      <div
    ...
    <>
      ...
      <AnimatePresence>
        {screenSize.greaterThanOrEqual("lg") && isPlaying && (
          <motion.div className="pointer-events-none absolute inset-0" exit={{scale: 0}} transition={{duration: 2, ease: "easeOut"}}>
            {[0, 1, 2, 3].map((index) => (
              <motion.div
                key={index}
      ...
      <div
        ...
        <img src="/welcome-help.png" alt="welcome-help" width={150} className="absolute right-[492px] bottom-28 brightness-75 contrast-125 invert max-md:hidden dark:invert-0" />
        ...
        <div className="relative z-50 size-full rounded-[inherit] p-2.5">
          <DPad className="bottom-[54px] left-[97px] max-sm:bottom-[60px] max-sm:left-[80px]" onClick={onDpadButtonClick} />
          <ActionButton className="bottom-[124px] left-[367px] max-sm:bottom-40 max-sm:left-[270px] max-sm:size-12" label="A" onClick={() => onActionButtionClick?.("A")} />
          <ActionButton className="bottom-[124px] left-[267px] max-sm:bottom-40 max-sm:left-[200px] max-sm:size-12" label="B" onClick={() => onActionButtionClick?.("B")} />

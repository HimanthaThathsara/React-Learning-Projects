...
export const PixelImage = ({
  ...
  return (
    <div ref={containerRef} className="relative">
        ...
        {pieces.map((piece, index) => (
          <div
            ...
            <img
              ...
              className={cn("size-full object-cover rounded-xl", grayscaleAnimation && (showColor ? "grayscale-0" : "grayscale"))}
              style={{
      ...
      {showReplayButton && (
        <button onClick={resetAnimation} className="absolute top-2 right-2 z-10 rounded-lg bg-black/50 px-3 py-1 text-xs text-white backdrop-blur-sm transition-opacity hover:bg-black/70">
          Replay
        </button>
      )}
    </div>

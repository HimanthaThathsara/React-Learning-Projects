...
const SignUpPage: React.FC<SignUpPageProps> = ({ onSignUp, onNavigateToLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In a real app, you'd validate and save user data here
    onSignUp();
  };

  return (
    <div className="bg-gray-50 dark:bg-[#1a1d21] text-gray-900 dark:text-gray-200 min-h-screen flex items-center justify-center p-4 transition-colors duration-200 w-full">
      <div className="w-full max-w-md bg-white dark:bg-[#222529] rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-8 pt-8 pb-6 text-center">
          <div className="mx-auto w-12 h-12 bg-blue-50 dark:bg-blue-900/20 rounded-lg flex items-center justify-center mb-4">
            <span className="material-icons text-blue-500 text-2xl">person_add</span>
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Create an account</h1>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Start your journey with us today.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5" htmlFor="confirm-password">Confirm Password</label>
            <div className="relative">
              <input 
                className="block w-full px-3 py-2.5 bg-white dark:bg-[#1a1d21] border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow sm:text-sm" 
                id="confirm-password" 
          ...
          <p className="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <button onClick={onNavigateToLogin} className="font-medium text-blue-600 hover:text-blue-700 transition-colors ml-1">Log in</button>
          </p>
        </form>

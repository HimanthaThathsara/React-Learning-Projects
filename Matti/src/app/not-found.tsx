...
const DesktopNavigationPanel = ({side, onClick, label, icon}: {side: "left" | "right"; onClick: () => void; label: string; icon: React.ReactNode}) => {
    ...
    <motion.div
      ...
      <div className="group relative h-35 w-full overflow-hidden">
        ...
        <div className="relative grid h-full w-full place-content-center">
          <Button variant="link" className="text-base transition-transform duration-200 group-hover:scale-105" onClick={onClick} aria-label={label}>
            {side === "left" && icon}
...
const MobileNavigation = ({onBack, onHome}: {onBack: () => void; onHome: () => void}) => {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 z-50 mx-auto w-full max-w-md px-4 pb-8 md:hidden">
      <motion.div initial={{opacity: 0, y: 50}} animate={{opacity: 1, y: 0}} transition={{delay: 0.6, duration: 0.5}} className="w-full">
        {/* Buttons */}
        <div className="relative flex flex-col gap-3">
          <motion.div custom={0} variants={buttonVariants} initial="initial" animate="animate">
            <Button variant="outline" onClick={onBack} className="group relative h-12 w-full overflow-hidden  border-dotted font-mono text-base shadow-lg" aria-label="Go back to previous page">
              <ArrowLeft className="mr-2 size-5 transition-transform group-hover:-translate-x-1" />
              <span>Go Back</span>
            </Button>
          </motion.div>

          <motion.div custom={1} variants={buttonVariants} initial="initial" animate="animate">
            <Button variant="ghost" size={"sm"} onClick={onHome} className="group relative h-12 w-full border-dotted border-2 overflow-hidden font-mono text-base" aria-label="Go to home page ">
      ...
      <motion.div initial={{opacity: 0}} animate={{opacity: 1}} transition={{delay: 0.9, duration: 0.5}} className="mt-4 text-center">
...
const NotFound = () => {
  ...
  return (
    <main className="relative h-screen overflow-hidden">
      ...
      {/*  404 Backdrop */}
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 flex items-center justify-center md:bottom-16">
      ...
      <Robot className="xs:w-[600px] md:xs:-bottom-40 md:xs:w-[700px] pointer-events-none absolute top-1/2 left-1/2 w-[500px] -translate-x-1/2 -translate-y-1/2 opacity-90 select-none sm:w-[700px] md:top-auto md:bottom-16 md:w-[800px] md:translate-y-0 md:sm:-bottom-50 lg:-bottom-92 lg:w-[1200px]">
        <div className="noise-screen grid place-content-center">
          <motion.h1
            ...
            <span className="font-bold text-red-500">404:</span> <Typewriter text={TYPEWRITER_MESSAGES} speed={70} waitTime={2000} deleteSpeed={50} cursorChar="_" />
          </motion.h1>
        </div>
      </Robot>

      {/*  Line */}
      <motion.div initial={{scaleX: 0}} animate={{scaleX: 1}} transition={{delay: 0.4, duration: 0.6}} className="absolute bottom-35 z-50 w-full border-b max-md:hidden" style={{originX: 0.5}} />

      <DesktopNavigationPanel side="left" onClick={handleGoBack} label="Go Back" icon={<ArrowLeft className="size-4" />} />

      <DesktopNavigationPanel side="right" onClick={handleGoHome} label="Go Home" icon={<ArrowRight className="size-4" />} />

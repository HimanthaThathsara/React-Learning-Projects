...
const processes = [
  {
    id: 1,
    title: " Sensor Design",
    description: "We begin by designing custom  sensors specifically for your industry and use case, selecting appropriate conductive materials and sensor types.",
  ...
  },
  {
    ...
    description: "We develop specialized algorithms that transform  sensor data into actionable insights unique to your industry requirements.",
  ...
  },
  {
    ...
    description: "We handle manufacturing, quality control, and ensure all  sensor products meet relevant industry standards and certifications.",
    steps: [" manufacturing partner selection", "Quality assurance processes", "Industry-specific certification procurement", "Initial production supervision"],
  },
  {
    id: 5,
    ...
    <section id="process" className="bg-white py-16">
      <div className="container mx-auto px-4" ref={processRef}>
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">We follow a structured approach to developing sensor solutions that ensures quality, reliability, and performance.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="sticky top-24 space-y-2">
              {processes.map((process) => (
                <button
                  ...
                  className={cn("w-full text-left p-4 rounded-lg transition-all", activeProcess === process.id ? "bg-gray-100 shadow-sm" : "hover:bg-gray-50")}>
                  <div className="flex items-center">
                    <div
                      className={cn(
                        "w-8 h-8 rounded-full flex items-center justify-center mr-3 transition-all",
                        activeProcess === process.id ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500"
                      )}>
                      {activeProcess > process.id ? <Check className="h-5 w-5" /> : <span>{process.id}</span>}
                    </div>
                    <span className={cn("font-medium", activeProcess === process.id ? "text-gray-900" : "text-gray-600")}>{process.title}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="md:col-span-8 lg:col-span-9">
            <div className="space-y-16">
              {processes.map((process) => (
                <div id={`process-${process.id}`} key={process.id} className="scroll-mt-24 transition-all duration-500" ref={(el) => (processSectionsRef.current[process.id - 1] = el)}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {process.steps.map((step, idx) => (
                      <div key={idx} className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        <div className="flex items-center">
                          <div className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center mr-3 text-sm">{idx + 1}</div>
                          <span>{step}</span>
                        </div>
                      </div>
                    ))}

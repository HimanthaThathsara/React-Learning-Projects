      ...
      <div className="container mx-auto max-w-6xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-12">
          <div>
            ...
            <p className="text-gray-800 max-w-xl">Explore our latest insights on Cell Tower Monitoring, industry trends, and innovation.</p>
          </div>
        ...
        <div className="relative">
            ...
            <div className="flex gap-6 pb-4 md:hidden overflow-x-auto snap-x snap-mandatory pl-1">
              {recentPosts.map((post) => (
                <div key={post.id} className="flex-none w-[85%] snap-center">
                  <BlogPostCard title={post.title} excerpt={post.excerpt} imageUrl={post.imageUrl || "/placeholder.svg"} date={post.date} slug={post.slug} category={post.category} />
                </div>
              ))}
            </div>
          </ScrollArea>
          ...
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {recentPosts.map((post) => (
              <BlogPostCard key={post.id} title={post.title} excerpt={post.excerpt} imageUrl={post.imageUrl || "/placeholder.svg"} date={post.date} slug={post.slug} category={post.category} />
            ))}
          </div>
          ...
          <div className="mt-4 flex justify-center md:hidden">
            <div className="flex gap-1">
              {[0, 1, 2].map((i) => (
                <div key={i} className={`h-1.5 rounded-full ${i === 0 ? "w-6 bg-gray-800" : "w-2 bg-gray-300"}`} />
              ))}
            </div>
          </div>
        </div>

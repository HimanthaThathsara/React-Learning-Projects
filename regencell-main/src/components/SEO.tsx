...
const SEO: React.FC<SEOProps> = ({
  ...
  type = "website",
  name = "RegenCell Technologies",
  imageUrl = "/Images/48ecf6e2-5a98-4a9d-af6f-ae2265cd4098.png",
  publishDate,
  modifiedDate,
  author,
  category,
  keywords = ["smarts", " technology", " sensors", " tech", "safety monitoring", "performance analytics"],
  isBlogPost = false,
}) => {
  const location = useLocation();
  const currentUrl = `https://RegenCell.com${location.pathname}`;
  const absoluteImageUrl = imageUrl.startsWith("http") ? imageUrl : `https://RegenCell.com${imageUrl}`;
  ...
  const enhancedKeywords = location.pathname.includes("smart-ppe-revolution")
    ...
    : location.pathname.includes("-safety-tech-protecting-workers-roi")
    ? [
        ...keywords,
  ...
  const organizationStructuredData = {
    ...
    "@type": "Organization",
    name: "RegenCell Technologies",
    url: "https://RegenCell.com",
    logo: "https://RegenCell.com/Images/14ea3fe0-19d6-425c-b95b-4117bc41f3ca.png",
    description: "Pioneering smart engineering solutions with  sensors",
    contactPoint: {
      "@type": "ContactPoint",
      contactType: "customer service",
      email: "info@RegenCell.com",
    },
    sameAs: ["https://www.linkedin.com/company/RegenCell-technologies", "https://twitter.com/RegenCelltechnologies"],
  };
  ...
  const blogPostStructuredData =
    isBlogPost && publishDate
      ? {
          ...
          author: {
            "@type": "Organization",
            name: author || "RegenCell Technologies",
            url: "https://RegenCell.com",
          },
          publisher: {
            "@type": "Organization",
            ...
            logo: {
              "@type": "ImageObject",
              url: "https://RegenCell.com/Images/14ea3fe0-19d6-425c-b95b-4117bc41f3ca.png",
              width: 512,
              height: 512,
            },
            url: "https://RegenCell.com",
  ...
  // Add FAQ structured data for  Safety Tech ROI post
  const SafetyROIFAQData = location.pathname.includes("-safety-tech-protecting-workers-roi")
    ? {
        "@context": "https://schema.org",
        "@type": "FAQPage",
        mainEntity: [
          ...
          {
            "@type": "Question",
            name: "What ROI can  safety technology deliver?",
            acceptedAnswer: {
              "@type": "Answer",
          ...
          {
            "@type": "Question",
            name: "Do insurance companies support  safety technology?",
            acceptedAnswer: {
              "@type": "Answer",
              text: "Yes, many insurers now bundle  device costs into workers compensation premiums. Employers keep the hardware as long as usage stays high because fewer claims leave insurers ahead financially. Regional carriers are expanding similar rebate schemes.",
            },
          },
        ],
      }
  ...
  const keywordString = category ? [...enhancedKeywords, category.toLowerCase()].join(", ") : enhancedKeywords.join(", ");
  ...
  return (
    <Helmet>
      ...
      <meta property="og:site_name" content="RegenCell Technologies" />
      <meta property="og:locale" content="en_US" />
      {isBlogPost && category && <meta property="article:section" content={category} />}
      {isBlogPost && publishDate && <meta property="article:published_time" content={publishDate} />}
      {isBlogPost && modifiedDate && <meta property="article:modified_time" content={modifiedDate} />}
      {isBlogPost && <meta property="article:publisher" content="https://RegenCell.com" />}
      ...
      <meta name="twitter:site" content="@RegenCelltechnologies" />
      <meta name="twitter:creator" content="@RegenCelltechnologies" />
      ...
      {blogPostStructuredData && <script type="application/ld+json">{JSON.stringify(blogPostStructuredData)}</script>}

      {smartPPEFAQData && <script type="application/ld+json">{JSON.stringify(smartPPEFAQData)}</script>}

      {SafetyROIFAQData && <script type="application/ld+json">{JSON.stringify(SafetyROIFAQData)}</script>}
    </Helmet>
  );
};
